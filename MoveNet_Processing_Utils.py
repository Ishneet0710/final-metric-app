...
def movenet_processing(frame, max_people=1, mn_conf=0.2, kp_conf=0, pred_conf=0, draw_movenet_skeleton = True, blur_faces = True, insightface = False):
    '''
    ...
    Args:
        ...
        - insightface: Whether or not to use InsightFace
    '''
        ...
        if confidence > mn_conf:
            ...
            if blur_faces:
                if not insightface:
                    ...
                    face_box_time, blurring_time = blur_faces_insightface(frame)
                    if face_box_time == -1:
                        return frame, -1, -1, -1
            else:
                face_box_time = 0
    ...
    try:
        rendering_time = draw_time_1 + blurring_time + draw_time_2                      # Rendering
        classifying_time = movenet_time + model_time                                    # Action Recogniton
        return frame, rendering_time, classifying_time, face_box_time     
